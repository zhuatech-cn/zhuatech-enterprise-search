...
package cn.zhuatech.enterprisesearch.config;
import cn.zhuatech.enterprisesearch.domain.DomainCatalog;
import cn.zhuatech.enterprisesearch.model.*;
import cn.zhuatech.enterprisesearch.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "ESEARCH-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="ESEARCH-DEMO-"+String.format("%03d",sequence);

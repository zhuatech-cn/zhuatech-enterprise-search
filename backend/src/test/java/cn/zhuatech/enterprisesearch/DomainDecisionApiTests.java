...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"queryNo\":\"QRY-2026-1008\",\"retrievedDocuments\":10,\"relevantDocuments\":8,\"citedDocuments\":8,\"permissionViolations\":0,\"latencyMs\":680,\"freshnessDays\":1}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"queryNo\":\"QRY-2026-1008\",\"retrievedDocuments\":20,\"relevantDocuments\":2,\"citedDocuments\":0,\"permissionViolations\":1,\"latencyMs\":5000,\"freshnessDays\":60}"))
